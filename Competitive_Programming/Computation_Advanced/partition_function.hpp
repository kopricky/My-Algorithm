...
vector<int> partition_function(const int n){
    vector<int> a(n + 1, 0);
    a[0] = 1;
    for(long long i = 1; i <= n; ++i){
        const ll p = i * (3 * i + 1) / 2, q = i * (3 * i - 1) / 2;
        if(p <= n) a[p] = add(a[p], (i % 2) ? MOD - 1 : 1);
        if(q <= n) a[q] = add(a[q], (i % 2) ? MOD - 1 : 1);
    }
    return polynomial_inverse(a, n + 1);
}